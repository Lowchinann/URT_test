...
protected:
  ...
  Trajectory armTrajectoryArmShelf_;
  Trajectory armTrajectoryMidpoint_1_;
  Trajectory armTrajectoryMidpoint_2_;
  Trajectory armTrajectoryMidpoint_3_;
  Trajectory armTrajectoryAfterUnload_;
...
public:
  ...
  bool initAction(){
    ...
    pnh_.getParam("unloadTable_enable", enable_move);
  ...
  StateEnum moveArmJoints( Trajectory traj ){
    if(!is_arm_sent_goal_){
      youbot_arm_joints::JointGoal goal;
      std::vector<double> joints;
      joints = traj[0].positions;
      goal.timeout = 20;
      goal.name = "arm";
      goal.joints = joints;
      client_arm_joints_->sendGoal(goal);
    ...
    StateEnum moveArmUpDownIk(aricc_vision_msgs::Object object,
      int dir, bool move_arm = true){
        ...
        if(found_1 != std::string::npos){
          ...
          //pos.y += offset_y_;
          pos.y = 0.0;
          //change the value to be added to offset_z_ to adjust dropping height for container
          pos.z = offset_z_ + 0.100;
...
//------------------------------------------------------------------------------
                ...
                sleep(2);
                state = findItemAndContainer( task_.items, best_items_, best_containers_ );
...
//------------------------------------------------------------------------------
                ...
                if(best_slot_ == 0)
                state = moveArmJoints(armTrajectoryMidpoint_1_);
                if(best_slot_ == 1)
                state = moveArmJoints(armTrajectoryMidpoint_2_);
                if(best_slot_ == 2)
                state = moveArmJoints(armTrajectoryMidpoint_3_);
                if(state == SUCCEEDED){
...
//------------------------------------------------------------------------------
                case kArmUnload:
                if(best_slot_ == 0)
                state = moveArmJoints(armTrajectoryLeaveTray_1_);
                if(best_slot_ == 1)
                state = moveArmJoints(armTrajectoryLeaveTray_2_);
                if(best_slot_ == 2)
                state = moveArmJoints(armTrajectoryLeaveTray_3_);
                if(state == SUCCEEDED){
                  sleep (1.0);
...
//------------------------------------------------------------------------------
                ...
                if(best_slot_ == 0)
                state = moveArmJoints(armTrajectoryReadyLeaveTray_1_);
                if(best_slot_ == 1)
                state = moveArmJoints(armTrajectoryReadyLeaveTray_2_);
                if(best_slot_ == 2)
                state = moveArmJoints(armTrajectoryReadyLeaveTray_3_);
                if(state == SUCCEEDED)
                task_.now = kArmReadyBeforeLoadTable;
                break;
//------------------------------------------------------------------------------
                case kArmReadyBeforeLoadTable:
                state = moveArmJoints(armTrajectoryNavigationPose_);
                if(state == SUCCEEDED)
...
//------------------------------------------------------------------------------
                ...
                if(state == SUCCEEDED)
                task_.now = kOpenGripperLoadTable;
                break;
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
                case kArmStandby:
                state = moveArmJoints(armTrajectoryLookTable_);
                if(state == SUCCEEDED) {

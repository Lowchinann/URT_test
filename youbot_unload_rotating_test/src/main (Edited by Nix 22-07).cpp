...
bool isBaseMoveEnoughLeft(double dist = 0){
  ...
  StateEnum taskStep(){
    ...
    switch(task_.now){
      ...
      case kArmLookTable:
       ...
       state = moveArmJoints(armTrajectoryLookTable_);
        if(state == SUCCEEDED){
          if(table_height_ == 0 || table_height_ == 0.20){
             stop = 0;
             numofmoveleft = 0;
             task_.now = kForwardAlignZero;
          }
          else {
	    stop =0;
            numofmoveleft =0;
	...
	if(table_height_ == 0.20) basePose.x =0.07;
        else if(table_height_ == 0.05) basePose.x = 0.05;
        else basePose.x = 0.0;
	...
	break;
      ...
      case kBaseAlignObject:
        ...
        //for rtt no alignment
        if(table_height_ == 0.20) basePose.y = 0.0;
        else basePose.y = best_containers_[0].position.y + offset_y_;
	...
	if(state == SUCCEEDED){
      ...
      case kBaseMoveLeft:
        //Sydney
        if(table_height_ == 0) basePose.y = 0.05;
        else basePose.y = 0.10;

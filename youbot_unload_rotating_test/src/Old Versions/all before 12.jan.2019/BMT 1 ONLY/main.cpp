// WARNING= THIS CODE IS FOR BMT 1 ONLY.This version is without tracking table. it will just unload whatever on it's back.
...
protected:
  ...
  std::vector<std::string> emptytray;
  ...
  Trajectory armTrajectoryBmtUnload_1_;
  Trajectory armTrajectoryBmtUnload_2_;
  Trajectory armTrajectoryBmtUnload_3_;
...
public:
  ...
  bool loadAllArmTrajectories(){
    ...
    if(!loadArmTrajectory("arm_trajectory_ready_leave_tray_3",armTrajectoryReadyLeaveTray_3_))
      return false;
    if(!loadArmTrajectory("arm_trajectory_bmt_unload_1",armTrajectoryBmtUnload_1_))
      return false;
    if(!loadArmTrajectory("arm_trajectory_bmt_unload_2",armTrajectoryBmtUnload_2_))
      return false;
    if(!loadArmTrajectory("arm_trajectory_bmt_unload_3",armTrajectoryBmtUnload_3_))
      return false;
    return true;
  }
  ...
  StateEnum taskStep(){
	...
	task_.now = kStaticFindObject;
      ...
      case kStaticFindObject:
         ...
         if(tray_state_==emptytray) task_.now = kFinishTask;
	   
	 else{
	 	for(int i=0;i<3;i+=1)
	   	{
	    	if(tray_state_.at(i)!="empty")
			{is_arm_sent_goal_ =0;
			best_slot_=i;
			tray_state_.at(i)="empty";
      ...
      case kArmMidPointBeforeUnload:
        ...
        if(best_slot_ == 0)
          state = moveArmTrajectory(armTrajectoryMidpoint_1_);
        if(best_slot_ == 1)
          state = moveArmTrajectory(armTrajectoryMidpoint_2_);
        if(best_slot_ == 2)
          state = moveArmTrajectory(armTrajectoryMidpoint_3_);
        if(state == SUCCEEDED){
  ...
  /*  ////here arm shelf code//// */
      ...
      case kArmLoadTable:
        if(best_slot_ == 0) 
          {
	 state = moveArmTrajectory(armTrajectoryBmtUnload_1_);	
	}
        if(best_slot_ == 1)
	{ 
          state = moveArmTrajectory(armTrajectoryBmtUnload_2_);
	}
        if(best_slot_ == 2) 
	{
          state = moveArmTrajectory(armTrajectoryBmtUnload_3_);
	}
	...
	task_.now = kArmLookTable;
      ...
      case kUpdateBestList:
        ...
        if(best_containers_.size() == 0 &&
          ...
          task_.now = kArmMidPointBeforeUnload;//kCheckArmIkBeforeGrasp;
